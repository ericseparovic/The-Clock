        ...
        if auth.validation_form_login(email, password) == True:
            ...
            if existsUser == True:
                ...
                if user:
                    ...
                    if rol == 2:
                        dataPersonal = personal.get_data_personal(idUser)
                        return jsonify(dataPersonal), 200
                else:
...
@app.route('/mark_start/<idPersonal>', methods=['POST'])
def insert_mark_start(idPersonal):
...
#marca hora salida
@app.route('/mark_end/<idPersonal>', methods=['POST'])
def insert_mark_end(idPersonal):
    #Se verifica que sea un metods POST
    print('markkkkkkkkkkkkkk ennnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn appp')
    if request.method == 'POST':
...
def insert_authorized_absences(idPersonal):
    ...
    if request.method == 'POST':
        if absences.validation_form_absences(startDate, endDate, reason) == True:
            ...
            return absences.validation_form_absences(startDate, endDate, reason)
...
@app.route('/get_authorized_absence/<idPersonal>', methods=['GET'])
def get_authorized_absence(idPersonal):
    if request.method == "GET":
        DT = datetime.now()
        currentDate = DT.strftime('%d/%m/%Y')
        result = absences.get_authorized_absence(idPersonal, currentDate)
        if result == []:
...
def get_marks():
    ...
    if request.method == 'GET':
        ...
        idCompany = data['idCompany']
        if startDate == '':
            return 'Fecha desde es requerida', 412
        if endDate == '':
            return 'Fecha hasta es requerida', 412
        if document == '':
        ...
        result = mark.get_marks(startDate, endDate, document, idCompany)
        if result == False:
            return jsonify('Documento incorrecto'), 409
      
        if result:
            return jsonify(result), 200
        if result == []:
            return jsonify('No hay registro de marcas'), 412
...
#API Llegadas tardes del dia
@app.route('/get_late_arrivals/<idCompany>',methods=['GET'])
def get_late_arrivals(idCompany):
    if request.method == 'GET':
        ...
        result = mark.get_late_arrivals(currentDate, idCompany)
        if result:
            return jsonify(result), 200
        else:
            return jsonify([]), 412
...
#API Salidas anticipadas
@app.route('/get_early_departure/<idCompany>',methods=['GET'])
def get_early_departure(idCompany):
    if request.method == 'GET':
        ...
        result = mark.get_early_departure(currentDate, idCompany)
        if result:
            return jsonify(result), 200
        else:
            return jsonify([]), 412
